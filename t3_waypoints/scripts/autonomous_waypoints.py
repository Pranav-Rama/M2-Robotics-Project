...
class NavTest():
    def __init__(self):
        ...
        #waypoints
        locations['waypoint_1'] = Pose(Point(0.266694557546, 2.02686812866, 0.00), Quaternion(0.000, 0.000, -0.00387220340739, 0.999992502992))
        locations['waypoint_2'] = Pose(Point(0.99999948937, -2.14577603054, 0.000), Quaternion(0.000, 0.000, -0.00149625181896, 0.999998880615))
        locations['waypoint_3'] = Pose(Point(0.999998880615, 0.527852533338, 0.000), Quaternion(0.000, 0.000, -0.00992117199984, 0.999950783962))
        ...
        while (not rospy.is_shutdown()) and (not n_successes == 3):
    ...
    #To stop after three waypoints
        else:
            #if n_successes == 3:
            #self.shutdown
            #self.move_base.cancel_goal()
            rospy.loginfo('##################################')
            rospy.loginfo('##### REACHED Three Waypoint #####')
            rospy.loginfo('##################################')
            rospy.on_shutdown(self.shutdown)
            exit()
            #self.control_c = True
